...
bool isRootScope(ast::TreePtr &scope) {
...
bool isT(ast::TreePtr &expr) {
    ...
    return t != nullptr && t->cnst == core::Names::Constants::T() && isRootScope(t->scope);
}
...
bool isChalkODMDocument(ast::TreePtr &expr) {
    ...
    return chalk != nullptr && chalk->cnst == core::Names::Constants::Chalk() && isRootScope(chalk->scope);
}
